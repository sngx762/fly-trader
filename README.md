...
```text
FlyA: +11.54%, WinRate 55.6%, сделок 169 (Побед 94, Поражений 75), Пик: $117.20
FlyB: +11.64%, WinRate 56.0%, сделок 168 (Побед 94, Поражений 74), Пик: $118.10
```

### XAUUSD (Twelve Data, 1h, 1000 свечей)
```text
FlyA: -0.64%, WinRate 50.3%, сделок 163 (Побед 82, Поражений 81), Пик: $107.22
FlyB: -2.37%, WinRate 50.0%, сделок 164 (Побед 82, Поражений 82), Пик: $105.91
```

> ⚠️ На золоте муха не показывает положительного edge на текущих hyperparams (обучение с нуля). Это честный результат: волатильность XAU в ~5 раз ниже BTC, дофаминовый RPE-сигнал слабее, и модель не находит стабильных паттернов в 1000 свечах. Возможные направления улучшения: тюнинг FEATURE_WINDOW, разные learning rates для низковолатильных активов, больше исторических данных.

> **Примечание:** Сравнение результатов в лоб (`BTC 24/7` vs `XAU 24/5`) некорректно из-за разной ликвидности, структуры торговых сессий и волатильности крипто- и сырьевых рынков.
