...
```text
FlyA: +11.36%, WinRate 47.7%, сделок 350 (Побед 167, Поражений 183)
FlyB: +11.79%, WinRate 48.9%, сделок 348 (Побед 170, Поражений 178)
```

> **⚠️ Внимание:** Результаты получены на синтетическом трендовом ряду (см. `market_data.py`). На реальных исторических данных (BTC/ETH с флэтами и разворотами) цифры будут скромнее — скачайте через `scripts/download_data.py` для честной оценки.
